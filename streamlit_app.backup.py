...
# --- IMAGES ---
# Default fallback path if not uploaded via UI
...
def get_image_as_base64_str(path, resize=True, max_size=(350, 350)):
    ...
    for catalogue_name, excel_path in CATALOGUE_PATHS.items():
        ...
        try:
            ...
            if 'IsNew' in df.columns:
                df['IsNew'] = pd.to_numeric(df['IsNew'], errors='coerce').fillna(0).astype(int)
            else:
                df['IsNew'] = 0
            ...
            if image_map:
                ...
                if excel_cat_key in image_map: matched_cat_map = image_map[excel_cat_key]
                else:
                    for k in image_map.keys():
                        if k in excel_cat_key or excel_cat_key in k:
                            matched_cat_map = image_map[k]; break
                ...
                if matched_cat_map:
                    for index, row in df.iterrows():
                        ...
                        if row_cat_key in matched_cat_map: matched_sub_map = matched_cat_map[row_cat_key]
                        else:
                            for k in matched_cat_map.keys():
                                if k in row_cat_key or row_cat_key in k: matched_sub_map = matched_cat_map[k]; break
                        
                        if matched_sub_map:
                            if row_item_key in matched_sub_map: found_path = matched_sub_map[row_item_key]
                            else:
                                best_score = 0; best_match_path = None
                                for img_key, path in matched_sub_map.items():
                                    score = fuzz.token_sort_ratio(row_item_key, img_key)
                                    if score > best_score: best_score = score; best_match_path = path
                                if best_score > 70: found_path = best_match_path

                        if found_path:
                            b64_string = get_image_as_base64_str(found_path, resize=True, max_size=(300, 300)) 
                            if b64_string: df.loc[index, "ImageB64"] = b64_string
            
            df.drop(columns=['index'], inplace=True, errors='ignore')
            all_data.append(df[required_output_cols])
        except Exception as e: st.error(f"Error processing catalogue {catalogue_name}: {e}"); continue 
...
def generate_story_html(story_img_1_b64):
    ...
    if story_img_1_b64:
        ...
        img_tag = '<div style="border: 2px dashed red; padding: 20px; color: red;">IMAGE NOT FOUND. Please upload the image in the sidebar "Customization" section.</div>'

    html = f"""
    <div class="story-page" style="page-break-after: always; padding: 25px 50px; /* Top/Bottom: 25px, Left/Right: 10px */ font-family: sans-serif; overflow: hidden; height: 280mm;">
...
def generate_pdf_html(df_sorted, customer_name, logo_b64, case_selection_map, custom_journey_img=None):
    # --- 1. DEFINE PATHS ---
    ...
    # --- LOGIC FOR JOURNEY IMAGE ---
    story_img_1_b64 = ""
    if custom_journey_img is not None:
        # If user uploaded via sidebar, use it
        try:
            bytes_data = custom_journey_img.getvalue()
            # Convert bytes to base64 directly
            story_img_1_b64 = base64.b64encode(bytes_data).decode()
        except Exception as e:
            st.error(f"Error processing uploaded image: {e}")
    else:
        # Fallback to local file if it exists
        story_img_1_b64 = load_img_robust("image_c1bc45.png", specific_full_path=STORY_IMG_1_PATH, resize=True, max_size=(600,600))
    ...
    CSS_STYLES = f"""
        ...
        /* COVER PAGE: Reduced height to 260mm to prevent overflowing onto a blank page */
        .cover-page {{ 
            width: 210mm; 
            height: 260mm; 
        ...
        /* STORY PAGE: Reduced height to 260mm */
        .story-page {{ 
            width: 210mm; 
            height: 260mm; 
        ...
        .toc-page {{ 
            width: 210mm; 
            min-height: 200mm; /* Flexible height */
    ...
    # 2. Journey (Updated text and image via generate_story_html)
    html_parts.append(generate_story_html(story_img_1_b64))
    
    # 3. Index (Intro page removed)
    ...
    for index, row in df_sorted.iterrows():
        ...
        if row['Category'] != current_category:
            ...
            # Note the #main-index link here matches the ID in the TOC function
            html_parts.append(f'''
    ...
    for idx, row in df_sorted.iterrows():
        ...
        if cat in case_selection_map:
            ...
            for k in case_data.keys():
                ...
                if "cbm" in k.lower(): 
                    try: 
                        # --- MODIFIED: Round to 3 decimal places ---
...
if True: 
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        # --- FIXED: ADDED CUSTOMIZATION UPLOADER ---
        st.markdown("### 🖼️ Customization")
        st.info("Upload the Journey Image here if it is missing.")
        uploaded_journey_img = st.file_uploader("Upload Journey Section Image", type=['png', 'jpg', 'jpeg'])
            ...
            if search_term:
                ...
                with col_filter:
                    ...
                    if sel_cat != NO_SELECTION_PLACEHOLDER: 
                        ...
                        # --- UPDATED LOGIC: Per-Category Subcategory Selection ---
                        if sel_cats_multi:
                            filtered_dfs = [] # We will collect filtered data for each category here
                            
                            st.markdown("---")
                            st.markdown("**Sub-Category Options:**")
                            
                            # Loop through each selected category to show specific filters
                            for category in sel_cats_multi:
                                # Get data just for this category
                                cat_data = catalog_subset_df[catalog_subset_df['Category'] == category]
                                
                                # Get clean subcategories (excluding N/A)
                                ...
                                if clean_subs:
                                    ...
                                    # Filter this category's data:
                                    # Keep rows where Subcategory is in selection OR it is N/A (always keep items without subcats)
                                    ...
                                    # If no subcategories exist for this category, keep all its data
                                    filtered_dfs.append(cat_data)
                            
                            # Combine all the specific filtered chunks back together
                            if filtered_dfs:
                ...
                with col_btns:
                    ...
                    if st.button("Clear Filters", use_container_width=True):
                        clear_filters_dropdown()

                st.markdown("---")
                if sel_cat != NO_SELECTION_PLACEHOLDER:
                    if not st.session_state.selected_categories_multi: st.info("👆 Please select one or more **Categories**.")
                    else:
    ...
    with tab2:
        ...
        if st.session_state.cart:
            ...
            if cart_search: 
                cart_df = cart_df[cart_df['ItemName'].str.lower().str.contains(cart_search)]
            
            # Add Remove checkbox column
            cart_df['Remove'] = False
            
            # --- UPDATED VIEW: Included 'Catalogue' column here ---
            # Reordered to show: Catalogue -> Category -> Item Name -> Remove
            ...
            # Logic to remove items based on the "Remove" checkbox
            ...
            if st.button("Generate Catalogue & Order Sheet"):
                ...
                if CONFIG:
                    try:
                        ...
                        # --- PASSED THE UPLOADED FILE HERE ---
                        html = generate_pdf_html(df_final, name, logo, selection_map, custom_journey_img=uploaded_journey_img)
                        ...
                        options = {
                            ...
                            # Bottom margin accommodates the footer text AND our floating button
                            'margin-bottom': '15mm', 
                            
                            # The standard text provided by the PDF engine (Centers)
                            'footer-center': f'Prepared for: {name}   |   Page [page] of [toPage]',
                            'footer-font-name': 'Arial',
                            'footer-font-size': '8',
                            
                            # Push the text down slightly so it aligns with our button
                            'footer-spacing': '5',
                            
                            # Add a nice line to separate content
                            'footer-line': '', 
