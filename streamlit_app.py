...
def get_image_as_base64_str(url_or_path, resize=None, max_size=None):
    ...
    try:
        ...
        buffered = io.BytesIO()
        # Optimization: Convert to RGB and use JPEG for smaller memory footprint
        if img.mode in ("RGBA", "P"):
            img = img.convert("RGB")
        img.save(buffered, format="JPEG", quality=80) # Reduced quality slightly for RAM safety
...
# --- 8. DATA LOADING (MEMORY OPTIMIZED) ---
...
def load_data_cached(_dummy_timestamp):
    ...
    for catalogue_name, excel_path in CATALOGUE_PATHS.items():
        ...
        try:
            # FIX: Fill NaN immediately to prevent "float has no attribute strip" error
            ...
            df["ImageURL"] = ""  # MEMORY FIX: Store URL, not Base64
            ...
            if cloudinary_map:
                for index, row in df.iterrows():
                    ...
                    if found_url:
                        df.loc[index, "ImageURL"] = found_url # Store URL only
...
def generate_table_of_contents_html(df_sorted):
    ...
    for category in unique_categories:
        ...
        rep_image = ""
        # MEMORY FIX: Check for ImageURL instead of ImageB64
        for _, row in group.iterrows():
            url = row.get('ImageURL', '')
            if url: 
                # JIT Conversion for TOC
                rep_image = get_image_as_base64_str(url, resize=(150,150))
                break 
