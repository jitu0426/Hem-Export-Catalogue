...
try:
    ...
    # --- 3. HELPER FUNCTIONS (MEMORY OPTIMIZED) ---
    def get_image_as_base64_str(url_or_path, resize=None, max_size=(300, 300)): # DEFAULT TO SMALL SIZE
        """
        Fetches, Resizes, and Compresses images to prevent Server Crashes (OOM).
        """
        ...
        try:
            ...
            # 2. Aggressive Resize (Critical for RAM)
            # If no specific resize is requested, default to max 300x300 (plenty for PDF cards)
            if max_size:
            ...
            if img.mode in ("RGBA", "P"):
                img = img.convert("RGB") # Drop alpha channel to save space
            
            # Save as JPEG with 60% quality (Visuals look fine, Size drops by 80%)
    ...
    def generate_table_of_contents_html(df_sorted):
        ...
        for category in unique_categories:
            ...
            rep_image = "" 
            # MEMORY FIX: Check for ImageURL first, JIT convert later
    ...
    def generate_pdf_html(df_sorted, customer_name, logo_b64, case_selection_map):
        ...
        for index, row in df_sorted.iterrows():
            ...
            # 4. PRODUCT CARD
            # MEMORY FIX: JIT Conversion of ImageURL to Base64
